...
//
///**
// * 把TrajectoryEnriche类解耦分离
// * Special Edition for 孝汉应
// */
//public class StateTGCTime1stv5 {
...
//    static long carid;
//    static  long zadaoMileage=100000;
//    static Map<Pair<Long,Long>,Boolean> IfLaterMap= new ConcurrentHashMap<>();
//    //           carid  zhadaoMileages    是否有数据
//    static Map<Long,Pair<Boolean,Integer>> nowMap= new ConcurrentHashMap<>();
//    //    carid  是否在路上  数据缺失了几次
//    static Map<Long,Boolean> laterMap= new ConcurrentHashMap<>();
//
...
//    static {
//        try {
//            gantryAssign = new GantryAssignment("/home/ljj/孝汉应卡口.xlsx");
//        } catch (IOException e) {
//            e.printStackTrace();
...
//                .process(new TrajectoryEnricherv5(d1TimeInterval, gantryAssign));
//        pathTDataStream.flatMap(new FlatMapFunction<PathTData, Object>() {
...
//                                    if(PointMap.get(key).getDirection()==1&&PointMap.get(key).getMileage()>15270){tempMap.remove(key);}
//                                    if(PointMap.get(key).getDirection()==2&&PointMap.get(key).getMileage()<15690){tempMap.remove(key);}
...
//                        }
//                    }
//                    try (BufferedWriter writer = new BufferedWriter(new FileWriter("D:\\learn\\codes\\a_idea_codes\\flinkTest\\src\\main\\java\\whu\\edu\\ljj\\flink\\data\\my.txt",true))) {
//                        //mark：用文件数据测试断连
