...
/**
 * Special Edition v6 for 孝汉应
 * -> 只根据数据自带时间
 */
// 只是在不实际分区的情况下适用，因为到处都是调用函数，所以状态一定全进程共享
public class TrajectoryEnricherv6 extends CoProcessFunction<PathTData, GantryData, PathTData> implements Serializable{

    private long timeInterval;
    private GantryAssignment gantryAssign;

    TrajectoryEnricherv6(long timeInterval, GantryAssignment gantryAssign) {
...
//        System.out.println("\n数据传入到匹配算子的时间："+System.currentTimeMillis());
...
//        System.out.println("输出的时间："+System.currentTimeMillis());
...
//                return;
        ...
        long matchTime = gantryTimestamp + 400 - timeInterval;
