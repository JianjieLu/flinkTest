...
public class DealWithJiZhanYuan100 {
...
//读取基站数据，返回StationStream
            DataStream<PathTData> StationStream=kafkaStream.flatMap((String jsonStr, Collector<PathTData> out)-> {
                try {
                    System.out.println("前一个时间戳："+System.currentTimeMillis());
            ...
            System.out.println("out split: "+(st4-st1));
...
// 创建第二个KafkaSink（新集群）
        KafkaSink<String> secondarySink = KafkaSink.<String>builder()
                .setBootstrapServers("100.65.155.40:9092") // 新增集群
                .setRecordSerializer(
                        KafkaRecordSerializationSchema.builder()
                                .setTopic("MergedRampPathData")
                                .setValueSerializationSchema(new SimpleStringSchema())
