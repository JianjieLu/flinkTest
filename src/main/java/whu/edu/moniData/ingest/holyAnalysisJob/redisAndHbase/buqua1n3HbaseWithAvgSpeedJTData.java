        ...
        public class buqua1n3HbaseWithAvgSpeedJTData {
                        ...
                        .flatMap(new RichFlatMapFunction<PathTData, IndicatorResult>() {
                            ...
                            public void flatMap(PathTData pathTData, Collector<IndicatorResult> collector) throws Exception {
                                ...
                                for (PathPoint point : pathList) {
                                    ...
                                    point.setPlateNo(String.valueOf(point.getId()));
                                    int direction = point.getDirection();
                                    int laneNo = point.getLaneNo();
                                    if(direction==2){
                                        if(laneNo==1)laneNo=5;
                                        if(laneNo==2)laneNo=6;
                                        if(laneNo==3)laneNo=7;
                                        if(laneNo==4)laneNo=8;
                                        if(laneNo==9)laneNo=10;
                                    }
...
//                                        if(timeRange == null) {
//                                            System.err.println("timeRange is null for key " + key);
//                                            return; // 或其他处理方式
//                                        }
                            ...
                            private IndicatorCalculationResult calculateIndicators(List<Pair<Long, Long>> timePairs, Pair<Long, Long> timeRange) {
                                ...
                                if(maxTime==minTime)result.timeRangeMs=1;
                                else result.timeRangeMs = maxTime - minTime;
...
//                                System.out.println("result.timeRangeMs"+result.timeRangeMs);
//                                System.out.println("result.totalOccupancy"+result.totalOccupancy);
//                                System.out.println("result.occupancy"+result.occupancy);
                                // 2. 计算车头时距
            ...
            public static class EnhancedIndicatorHBaseSink extends RichSinkFunction<IndicatorResult> {
                ...
                public void open(Configuration parameters) throws Exception {
                    ...
                    // 确保表存在
                    createTableIfNotExists(connection, "real_traffic_indicators", "cf");
                    table = connection.getTable(TableName.valueOf("real_traffic_indicators"));
                }
                ...
                public void invoke(IndicatorResult result, Context context) throws Exception {
                    try {
                        ...
                        // 调试：打印序列化前的对象
                        System.out.println("序列化前对象: " + indicators);

                        // 序列化为JSON
                        String jsonStr = JSON.toJSONString(indicators);
                        System.out.println("序列化后JSON: " + jsonStr);  // 添加调试输出

                        if (jsonStr == null || jsonStr.isEmpty()) {
                            System.err.println("JSON序列化失败，对象为空: " + rowKey);
                            return;
                        }
                        ...
                        // 写入HBase
                        table.put(put);
                        System.out.println("成功存储指标数据: " + rowKey + " | JSON长度: " + jsonStr.length());

                    } catch (Exception e) {
                        System.err.println("存储HBase失败: " + e.getMessage());
                        e.printStackTrace();
                    }
                }
