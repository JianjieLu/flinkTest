...
//    private static final KafkaProducerUtil producer3 = new KafkaProducerUtil(TOPICS[2]); // 第三段Topic
    ...
    private static final double DATA_POINT_MISSING_PROBABILITY = 0; // 1%数据点整体缺失概率
    private static final double ID_CHANGE_PROBABILITY = 0; // 0.01%车辆ID突变概率
    private static final double WAYNO_CHANGE_PROBABILITY = 0; // 10%车辆变道概率
...
//            long tpointno = 1049950; // 初始位置在MIN-MAX之间随机
    ...
    private static long generateInitialPosition(Random random) {
        ...
        long range = 900;
        return 1049000 + (long) (random.nextDouble() * range);
    }
