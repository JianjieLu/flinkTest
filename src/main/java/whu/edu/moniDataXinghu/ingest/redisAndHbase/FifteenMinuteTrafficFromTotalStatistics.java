...
public class FifteenMinuteTrafficFromTotalStatistics {
...
//        String topics = "fiberData1 fiberData2 fiberData3 fiberData4 fiberData5 fiberData6 fiberData7 fiberData8 fiberData9 fiberData10 fiberData11";
        ...
        // 6. 写入HBase
        trafficFlow.addSink(new FifteenMinuteTrafficHBaseSink("FifteenMinuteTrafficFlow", "cf"));

        env.execute("15-Minute Traffic Flow Analysis and Storage Job");
    }
    ...
    public static class VehicleDeduplicationProcess
        ...
        public void processElement(
            ...
            int truncatedMinute = (minute / 15) * 15; // 0, 15, 30, 45
            ZonedDateTime truncated = zdt.withMinute(truncatedMinute).withSecond(0).withNano(0);
            long windowTimestamp = truncated.toInstant().toEpochMilli();

            // 检查是否在同一15分钟窗口同一路段报告过
            Integer lastSection = lastSectionState.get(windowTimestamp);
            if (lastSection == null || lastSection != event.sectionId) {
                // 首次在该15分钟窗口该路段报告
                lastSectionState.put(windowTimestamp, event.sectionId);
                out.collect(event);
            }
            // 否则忽略重复报告
        }
    ...
    // HBase Sink - 修改为15分钟粒度
    public static class FifteenMinuteTrafficHBaseSink extends RichSinkFunction<Tuple8<Long, Integer, Long, Long, Long, Long, Double, Double>> {
        ...
        public FifteenMinuteTrafficHBaseSink(String tableName, String columnFamily) {
        ...
        public void invoke(Tuple8<Long, Integer, Long, Long, Long, Long, Double, Double> value, Context context) throws Exception {
            // 使用包含分钟的新格式
            String timeStr = Instant.ofEpochMilli(value.f0)
                    .atZone(ZoneId.systemDefault())
